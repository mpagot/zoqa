...
fn encodeDepsAndApplyOverrides(
    ...
    for (collected) |c| {
        lookup_entries.append(arena_alloc, .{
            ...
            .settings = c.settings,
            .name = c.name,
            .has_parents = c.parent_chained.len > 0 or c.parent_directly_chained.len > 0 or c.parent_parallel.len > 0,
        }) catch |err| {
    ...
    for (collected) |*entry| {
        ...
        final_jobs.append(arena_alloc, .{
            ...
            .name = entry.name,
            .has_parents = entry.parent_chained.len > 0 or entry.parent_directly_chained.len > 0 or entry.parent_parallel.len > 0,
        }) catch |err| {
...
/// - `retry_cfg`: Retry/timeout configuration. Transient pre-stream failures
///   (connection errors, 502/503) are retried by `openQARawGet`; 404 fails
...
fn downloadAssets(
    ...
    for (assets) |asset| {
        ...
        // Skip unpublished UEFI vars assets, unless we are self-healing
        // skipped parents. Policy lives in the library.
        const skipped_parents = clone_opts.skip_deps or clone_opts.skip_chained_deps;
        if (zoqa.clone_job.shouldSkipUefiVarsAsset(asset.filename, final_jobs, skipped_parents)) {
            if (verbose) {
                std.debug.print("Skipping unpublished UEFI vars asset: {s}\n", .{asset.filename});
            }
            continue;
        }
        ...
        if (stream_result.status != .ok) {
            // The helper has already deleted the partial file for us.
...
pub fn main() !void {
    ...
    } else try cli_env.resolveCredentials( // TODO: why two near consecutive calls to cli_env.resolveCredentials? They only use different value for the host_url argument. Why one use try and one use catch?
        gpa,
        resolved.host_url,
        args.apikey,
        args.apisecret,
