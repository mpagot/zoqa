...
pub const ScheduleOptions = struct {
    ...
    /// Default is 1s for schedule (vs 10s for standalone monitor).
    poll_interval: u64 = 1,
...
pub fn runSchedule(
    ...
    if (has_ids) {
        ...
        // a top-level `error` nor a non-empty `failed` array.
        // Print the empty-set note for visibility but do NOT
