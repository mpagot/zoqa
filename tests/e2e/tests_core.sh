...
echo "==> [COR] Running core protocol and CLI flag tests..."

# Test COR-1: Basic GET jobs/overview (verifies the endpoint responds)
run_comparison_api "GET jobs/overview" "" "jobs/overview" 0

# Test COR-2: GET workers
run_comparison_api "GET workers" "" "workers" 0

# Test COR-3: GET with query params
run_comparison_api "GET jobs with filter" "" "jobs distri=opensuse" 0 "\[\]"

# Test COR-4: GET 404: get intentionally not existing job id
run_comparison_api "GET non-existent (404)" "" "jobs/999999" 1 "404 Not Found"

# Test COR-5: Missing PATH positional argument
# Both Perl and Zig exit 255 and print the api usage block to stderr.
#   "openqa-cli api" without anything else
run_comparison_api "Missing PATH" "" "" 255

# Test COR-6: Invalid host (connection refused)
# Both Perl and Zig exit 1. Messages differ (Perl: ANSI-colored, Zig: plain),
# so no grep pattern is used. Intentionally use a valid host but an invalid port number
...
# Test COR-7: Flags placed before the subcommand name must be rejected (exit 255).
# run_comparison_api cannot be used: it always emits "$EXE api --host http://localhost
...
# Test COR-8: -- is silently accepted and has no practical effect.
...
run_comparison_api "-- stop before API path accepted (exit 0)" "" \
	"-- jobs/overview" 0

# Test COR-9: -- causes a dash-prefixed token to be used as the literal API path.
...
run_comparison_api "-- stop: dash-prefixed path is a 404, not a flag error" "" \
	"-- -X" 1 "404 Not Found"

# Test COR-10: --param-file support.
...
run_comparison_api "--param-file" "" "--param-file distri=/tmp/distri.txt jobs" 0 "\[\]"

# Test COR-10b: --param-file returns a non-empty result when the value matches seeded data.
...
run_comparison_api "--param-file with matching distri returns results" "" \
	"--param-file distri=/tmp/distri_example.txt jobs" 0 '"jobs"'

# Test COR-36: --data / -d raw body POST.
...
run_comparison_api "--data raw body POST to isos" "" \
...
# Test COR-36b: --data-file POST with a matching product schedules a job (count:1).
...
run_comparison_api "--data-file POST matching product schedules a job (count:1)" "" \
	"-X POST -a 'Content-Type: application/x-www-form-urlencoded' --data-file /tmp/body36b.txt isos" \
	0 '"count":1'

# Test COR-37: --form converts a JSON object body to application/x-www-form-urlencoded.
...
# shell evaluation chain used by run_comparison_api.
...
run_comparison_api "--form JSON→form-encoded POST to isos" "" \
	"-X POST --form --data-file /tmp/form37.json isos" \
	0

# Test COR-38: -a / --header injects a custom request header.
...
run_comparison_api "-a custom header does not break request" "" \
	"-a 'X-E2E-Test: zoqa' jobs/overview" \
	0

# Test COR-39: --json + --data-file + PUT.
...
# The JSON is written to a file to avoid double-quote nesting in run_comparison_api.
#
...
run_comparison_api "--json + --data-file + PUT jobs/:id" "" \
	"--json --data-file /tmp/put39.json -X PUT jobs/$JOB_ID" \
	0

# Test COR-40: --param-file combined with a positional key=value parameter.
...
run_comparison_api "--param-file + positional KV" "" \
	"--param-file version=/tmp/pf_version.txt jobs distri=opensuse" \
	0 "\[\]"

# Tests COR-41a-c: Host resolution and scheme handling.
# run_comparison_api cannot be used here because these tests supply a custom --host
...
# Test COR-41a: Bare hostname → https://.
...
# Test COR-41b: Unresolvable hostname → DNS failure (exit 1).
...
# Test COR-41c: Fully-qualified URL with explicit scheme + unused port → ECONNREFUSED.
...
# Test COR-42: Combined short flags (e.g. -vp) must not be treated as -v + -p.
...
# Test COR-43: Archive-specific flag --with-thumbnails must be rejected for api.
